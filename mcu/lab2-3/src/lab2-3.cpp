...
static void vReplyTask(void *pvParameters) {
	while (1) {
		if (xSemaphoreTake(semaphore, portMAX_DELAY) == pdTRUE) {
			guard.write("[Oracle] Hmmm...\r\n");
			vTaskDelay(configTICK_RATE_HZ * 3);
			guard.write("[Oracle] ");
			guard.write(list[xTaskGetTickCount() % TASK_SIZE]);
			guard.write("\r\n");

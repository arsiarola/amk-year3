...
static void send_task(void *pvParameters) {
	...
	while (1) {
		char str[] = "send something";
		/* int len = sprintf(str, "Counter: %lu runs really fast\r\n", count); */
		USB_send((uint8_t*)str, sizeof(str)/sizeof(str[0]));
...
int main(void) {
	...
	xTaskCreate(send_task, "Tx",
				configMINIMAL_STACK_SIZE + 32, NULL, (tskIDLE_PRIORITY + 1UL),
				(TaskHandle_t *) NULL);

	/* LED1 toggle thread */
	xTaskCreate(receive_task, "Rx",
				configMINIMAL_STACK_SIZE + 48, NULL, (tskIDLE_PRIORITY + 1UL),
				(TaskHandle_t *) NULL);

	/* LED2 toggle thread */
	xTaskCreate(cdc_task, "CDC",
				configMINIMAL_STACK_SIZE + 66, NULL, (tskIDLE_PRIORITY + 1UL),
				(TaskHandle_t *) NULL);
